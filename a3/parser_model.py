...
class ParserModel(nn.Module):
    ...
    def forward(self, w):
        ...
        h_ = torch.matmul(embeddings, self.embed_to_hidden_weight) + \
                self.embed_to_hidden_bias
        h = nn.functional.relu_(h_)
        h_dropout = self.dropout(h)
        logits = torch.matmul(h_dropout, self.hidden_to_logits_weight) + \
